...
class Fisica:    
    ...
    prov_nato = Column(String(20), nullable=True)
...
class Indirizzo:
    ...
    prov = Column(String(20), nullable=True)
    loc = Column(String(256), nullable=True)
    cap = Column(String(50), nullable=True)
...
class Richiedente(Base, Fisica, Indirizzo, Recapito):         
    ...
    giuridica_opt = Column(Integer, default=0)
    delegato_opt = Column(Integer, default=0)
    domicilio_opt = Column(Integer, default=0)
    ...
    sostituito = Column(Boolean, nullable=True, default=False)
    principale = Column(Boolean, nullable=True, default=False)

    istanza = relationship("Istanza", back_populates="richiedenti")
    domicilio = relationship("Domicilio", back_populates="richiedente")
    condominio = relationship("Condominio", back_populates="richiedente")
    giuridica = relationship("Giuridica", back_populates="richiedente", uselist=False)

    
    def __init__(self, giuridica=None, *args, **kwargs):
        super(Richiedente, self).__init__(*args, **kwargs)
        import pdb;pdb.set_trace()
        if giuridica:
            self.giuridica = Giuridica(**giuridica)

           
class Giuridica(Base,Indirizzo,Recapito):    
...
class Domicilio(Base,Indirizzo):    
